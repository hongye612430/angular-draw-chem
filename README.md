...
6. `DCShape` factory - defines `Shape` class, which encapsulates svg-relevant data,
7. `DCAtom` factory - defines `Atom` class, which encapsulates data about a single atom,
8. `DCStructure` factory - defines `Structure` class, which encapsulates a structure tree (how the atoms are connected with each other),
9. `DCStructureCluster` factory - defines `StructureCluster` class, which enables grouping of multiple `Structure` objects; used for predefined structures,
10. `DrawChemConst` factory - contains all constants, e.g. BOND_LENGTH, BOND_WIDTH, etc.
